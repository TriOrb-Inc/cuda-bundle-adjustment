...
- 線形解法が失敗した LM 試行では、解法器が増分 buffer を更新していないため、姿勢・ランドマークの `update` と候補誤差評価を行わない。対角成分と solution snapshot を復元して lambda を増やし、次の試行へ進む。これにより未初期化値または前回の増分を一時適用せず、既存の retry 回数と lambda 系列を維持する。
