...
- sparse Cholesky の `information` はfactorization試行ごとに更新する。LMはzero-pivot後に
  dampingを増やして同じsymbolic patternを再試行するため、前回の
  `NUMERICAL_ISSUE`を保持すると、正定値化した後続行列まで失敗扱いになりchi2不変で終了する。
